...
defmodule AdventOfCode.Days.Day16 do
  ...
  defp bitwise_child_packets(b, packets \\ []) do
    {p, rest} = packet(b)
    packets = [p | packets]
    if rest == <<>>, do: packets |> Enum.reverse(), else: bitwise_child_packets(rest, packets)
  end

  defp count_child_packets(b, target_count, packets \\ []) do
    {p, rest} = packet(b)
    packets = [p | packets]
  ...
  defp version(<<v::3, rest::bitstring>>), do: {v, rest}
  defp type(<<t::3, rest::bitstring>>), do: {to_type(t), rest}

  defp to_type(0), do: :sum
  defp to_type(1), do: :product
  defp to_type(2), do: :minimum
  defp to_type(3), do: :maximum
  defp to_type(4), do: :literal
  defp to_type(5), do: :gt
  defp to_type(6), do: :lt
  defp to_type(7), do: :eq

  defp value(<<c::1, v::4, rest::bitstring>>, segment_count \\ 1, current \\ <<>>) do
  ...
  defp child_length(<<t::1, len::15, rest::bitstring>>) when t == 0, do: {:bits, len, rest}
  defp child_length(<<t::1, len::11, rest::bitstring>>) when t == 1, do: {:count, len, rest}
  ...
  defp sum_versions(packet) do
    packet.version + (packet.children |> Enum.map(&sum_versions(&1)) |> Enum.sum())
  end
